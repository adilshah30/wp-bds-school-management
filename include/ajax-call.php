...
add_action('wp_ajax_nopriv_ajax_bds_teacher_add_subject', 'ajax_bds_teacher_add_subject');
add_action('wp_ajax_nopriv_ajax_bds_delete_subject', 'ajax_bds_delete_subject');
add_action('wp_ajax_nopriv_ajax_bds_teacher_edit_subject', 'ajax_bds_teacher_edit_subject');
//---- newsletter category---//
add_action('wp_ajax_nopriv_ajax_bds_teacher_add_news_cat', 'ajax_bds_teacher_add_news_cat');
//---- Downloads category---//
add_action('wp_ajax_nopriv_ajax_bds_teacher_add_download_cat', 'ajax_bds_teacher_add_download_cat');
//---- gallery category---//
add_action('wp_ajax_nopriv_ajax_bds_teacher_add_gallery_cat', 'ajax_bds_teacher_add_gallery_cat');
add_action('wp_ajax_nopriv_ajax_bds_delete_gallery_cat', 'ajax_bds_delete_gallery_cat');
//--------Cart-------//
add_action('wp_ajax_nopriv_ajax_bds_add_to_cart', 'ajax_bds_add_to_cart');
//---Rating---//
add_action('wp_ajax_nopriv_ajax_bds_add_rating', 'ajax_bds_add_rating');
//-- newsletter --//
add_action('wp_ajax_nopriv_ajax_bds_feature_newsletter', 'ajax_bds_feature_newsletter');
add_action('wp_ajax_nopriv_ajax_bds_delete_newsletter', 'ajax_bds_delete_newsletter');
add_action('wp_ajax_nopriv_ajax_bds_delete_newsletter_cat', 'ajax_bds_delete_newsletter_cat');
//-- Download --//
add_action('wp_ajax_nopriv_ajax_bds_delete_download_cat', 'ajax_bds_delete_download_cat');
...
add_action('wp_ajax_nopriv_ajax_teacher_send_reply_message_request','ajax_teacher_send_reply_message_request');
...
//-----------------------------------------------------------
//---------------Add to cart --------------//
function ajax_bds_add_to_cart(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_subject = $wpdb->prefix.'activities_orders';	
    //echo "Table Name : ".$postmeta_table;
    //$event_date = date('y-m-d h:i:s');
    //$Current_date = date('y-m-d h:i:s');
    $add = $wpdb->insert($bds_add_subject, array(            
...
//-----------------------------------------------------------
function ajax_bds_teacher_add_subject(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_subject = $wpdb->prefix.'bds_teacher_subjects';	
    //echo "Table Name : ".$postmeta_table;
    //$event_date = date('y-m-d h:i:s');
    //$Current_date = date('y-m-d h:i:s');
    $add = $wpdb->insert($bds_add_subject, array(            
            'subject_name'=>$bds_subject,
            'teacher_id'=> $teacher_id		
    ));
    if (false === $add) {
            echo json_encode(array('status' => 0));
...
function ajax_bds_teacher_edit_subject(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_subject = $wpdb->prefix.'bds_teacher_subjects';	

    $where = array('id' => $subject_id);
    $update = $wpdb->update($bds_add_subject, array(            
            'subject_name' => $bds_subject	
    ),$where);
    if (false === $update) {
            echo json_encode(array('status' => 0));
...
function ajax_bds_teacher_add_news_cat(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_news_cat = $wpdb->prefix.'bds_newsletter_category';	
    //echo "Table Name : ".$postmeta_table;
    //$event_date = date('y-m-d h:i:s');
    //$Current_date = date('y-m-d h:i:s');
    $add = $wpdb->insert($bds_add_news_cat, array(            
            'category_name'=>$bds_news_cat,
            'teacher_id'=> $teacher_id		
    ));
    if (false === $add) {
            echo json_encode(array('status' => 0));
...
function ajax_bds_teacher_add_download_cat(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_download_cat = $wpdb->prefix.'bds_downloads_category';	
    //echo "Table Name : ".$postmeta_table;
    //$event_date = date('y-m-d h:i:s');
    //$Current_date = date('y-m-d h:i:s');
    $add = $wpdb->insert($bds_add_download_cat, array(            
            'category_name'=> $bds_download_cat,
            'teacher_id'=> $teacher_id		
    ));
    if (false === $add) {
            echo json_encode(array('status' => 0));
...
function ajax_bds_teacher_add_gallery_cat(){
    extract($_POST);
    global $wpdb; 	
    $bds_add_gallery_cat = $wpdb->prefix.'bds_gallery_category';	
    //echo "Table Name : ".$postmeta_table;
    //$event_date = date('y-m-d h:i:s');
    //$Current_date = date('y-m-d h:i:s');
    $add = $wpdb->insert($bds_add_gallery_cat, array(            
            'category_name'=>$bds_gallery_cat,
            'teacher_id'=> $teacher_id		
    ));
    if (false === $add) {
            echo json_encode(array('status' => 0));
...
function ajax_bds_delete_gallery_cat(){
    global $wpdb; 
    
    if(isset($_POST['id'])){
        extract($_POST);
        
        $tbl_newsletter_cat = $wpdb->prefix.'bds_gallery_category';
        $sql = "DELETE FROM $tbl_newsletter_cat WHERE id ='".$_POST['id']."'";
...
function ajax_bds_feature_newsletter(){
    global $wpdb; 
    
    if(isset($_POST['id'])){
        extract($_POST);
        $tbl_newsletter = $wpdb->prefix.'news_letter';
        $where = array('id' => $id);
        
        $where_all = array('teacher_id'=>$teacher_id);
        $update_all = $wpdb->update($tbl_newsletter, array(            
                'is_home' => 0,		
        ),$where_all);
        
        
        $update = $wpdb->update($tbl_newsletter, array(            
                'is_home' => 1,		
        ),$where);	
...
function ajax_bds_delete_newsletter(){
    global $wpdb; 
    ...
    if(isset($_POST['id'])){
        ...
        $tbl_newsletter = $wpdb->prefix.'news_letter';
        $sql = "DELETE FROM $tbl_newsletter WHERE id ='".$_POST['id']."'";
        $result= $wpdb->query($sql);	
...
function ajax_bds_delete_subject(){
    global $wpdb; 
    
    if(isset($_POST['id'])){
        extract($_POST);
        
        $tbl_sub = $wpdb->prefix.'bds_teacher_subjects';
        $sql = "DELETE FROM $tbl_sub WHERE id ='".$_POST['id']."'";
        $result= $wpdb->query($sql);	
...
function ajax_bds_delete_newsletter_cat(){
    global $wpdb; 
    
    if(isset($_POST['id'])){
        extract($_POST);
        
        $tbl_newsletter_cat = $wpdb->prefix.'bds_newsletter_category';
        $sql = "DELETE FROM $tbl_newsletter_cat WHERE id ='".$_POST['id']."'";
        $result= $wpdb->query($sql);	
...
function ajax_bds_delete_download_cat(){
    global $wpdb; 
    
    if(isset($_POST['id'])){
        extract($_POST);
        
        $tbl_download_cat = $wpdb->prefix.'bds_downloads_category';
        $sql = "DELETE FROM $tbl_download_cat WHERE id ='".$_POST['id']."'";
        $result= $wpdb->query($sql);	
...
function ajax_bds_add_rating(){
    ...
    if(isset($_POST['rating'])){
        ...
        $tbl_rating = $wpdb->prefix.'ratings';
        $add = $wpdb->insert($tbl_rating, array(
                'rating_postid' => $post_id,
//              'rating_posttitle' => $session_id,
                'rating' => $rating,
                'rating_userid'=>$user_id
                		
        ));
        if (false === $add) {
                echo json_encode(array('data_rate' => 0));
                echo $wpdb->print_error();
        } else {
                echo json_encode(array('data_rate' => 1));
        } 
        
    }
    	//$change = json_encode(array('data_rate' => ));	
        //echo $change;
    die();
}
...
function ajax_bds_teacher_home_work_detail(){
    ...
    if(isset($_POST['id'])){
        ...
        $homework_detailss = $wpdb->get_row("SELECT $homework_table.*, $subject_table.subject_name FROM $homework_table INNER JOIN $subject_table ON $homework_table.subject_id = $subject_table.id WHERE $homework_table.id = '".$homework_id."'" );        
        //$result= $wpdb->query($sql);	
        if (false === $homework_detailss) {
                ...
                //echo json_encode(array('da' => 'charlie'));
        } 
    }
...
function ajax_teacher_add_event_request(){
	...
	$wpdb->insert('wp_dkf3k12nf1_posts', array(
		...
		'post_parent' => 0,
		'guid' => 'http://brookridgedayschool.com/?page_id=',
		'menu_order' => 0,
...
//
	...
	if (count($error) > 0) {
		foreach ($error as $value) {
            ...
            $to = $result->email;
            $subject = 'Invitation';
            $body = 'invitation  from $name login criteria look: http://brookridgedayschool.com/login/  username: $email, password: $password';
...
function ajax_invite_teacher(){
    ...
    $body = '<table><tr><td>Please click the link to accept invitation or copy the link in browser</td></tr> <br/><tr><td> <a href="'.$href.'" style="background-color: #83996f;
    color: #f7f6f4;
    text-shadow: 0px 1px #5d5d5d;
    font-size: 13px;
    font-family: Open Sans, serif;    padding: 9px 50px;"> Click to Register </a></td></tr></table>' ;
...
function ajax_reinvite_teacher(){
    ...
    $result = $result[0];
//print_r($result);
    $href = 'http://brookridgedayschool.com/teacher-registration/?full_name='.str_replace("=", "", base64_encode($result->full_name.'')).'&email='. str_replace( "=","", base64_encode($result->email) ).'&user='.str_replace("=", "", base64_encode( $result->id ) ).'';
//   send email
    //add_filter( 'wp_mail_content_type', 'wpdocs_set_html_mail_content_type' );
...
//                $body = 'Please click or copy the link to accept invitation ' .  $href;
//
//                wp_mail( $to, $subject, $body );    
//                // Reset content-type to avoid conflicts -- https://core.trac.wordpress.org/ticket/23578
//                remove_filter( 'wp_mail_content_type', 'wpdocs_set_html_mail_content_type' );    
...
//   send email
    ...
    $body = '<table><tr><td>Please click the link to accept Invitation or copy the link in browser</td></tr> <br/><tr><td> <a href="'.$href.'" style="background-color: #83996f;color: #f7f6f4;
    text-shadow: 0px 1px #5d5d5d;
    font-size: 13px;
    font-family: Open Sans, serif;    padding: 9px 50px;"> Click to Register </a></td></tr></table>' ;
...
//print_r($_POST);
    ...
    $result = $result[0];
//print_r($result);
    $href = 'http://brookridgedayschool.com/roster/?full_name='.str_replace("=", "", base64_encode($result->full_name.'')).'&email='. str_replace( "=","", base64_encode($result->email) ).'&user='.str_replace("=", "", base64_encode( $result->id ) ).'';
//   send email
    //add_filter( 'wp_mail_content_type', 'wpdocs_set_html_mail_content_type' );
    ...
    $subject = 'Invitation';
    $headers = array('Content-Type: text/html; charset=UTF-8');
    $body = '<table><tr><td>Please click the link to accept invitation</td><tr/> <br/><tr><td> <a href="'.$href.'" style="background-color: #83996f;
    color: #f7f6f4;
    text-shadow: 0px 1px #5d5d5d;
    font-size: 13px;
    font-family: Open Sans, serif;    padding: 9px 50px;"> Click to Register </a></td><tr/></table>' ;
		...
		if(count($error) > 0){
			...
			if(!empty($_POST['caption'])) {
				...
				$movefile = wp_handle_upload($uploadedfile, $upload_overrides);
                                ...
                                //Update all student week records
                                $where_student = array('teacher_id' => $teacher_id);
                                $update_student_week= array(
                                    'student_of_week' => 0
                                );
                                $updated_std_week = $wpdb->update( $table_student, $update_student_week, $where_student );
				...
				$update =  array(
					...
					'movie'  => $movie,
                                        'hero' => $hero,
					'student_of_week' => $student_of_week,
					'sport' => $support,
					'status' => '1'
				);
			if(!empty($_POST['caption'])) {
...
function ajax_teacher_send_message_request(){
	...
	if(isset($_SESSION['teacher'])){
		$from = 't_'.$_SESSION['teacher'];
                $teacher_user_detail = $wpdb->get_row("SELECT * FROM $tbl_teacher WHERE id = '".$_SESSION['teacher']."'" );
                $send_from_name = $teacher_user_detail->full_name;
                $send_from_email = $teacher_user_detail->email;
                
	}
	elseif($_SESSION['parent']){
		$from = 'p_'.$_SESSION['parent'];
                $parent_user_detail = $wpdb->get_row("SELECT * FROM $tbl_parent WHERE id = '".$_SESSION['parent']."'" );
                $send_from_name = $teacher_user_detail->full_name;
                $send_from_email= $teacher_user_detail->email;
	}
...
//	roster_id
	if(!empty($roster_id)){
		foreach( explode(',', $roster_id) as $value){
                        ...
                        $parent_user_detail = $wpdb->get_row("SELECT * FROM  $tbl_parent WHERE id = '".$value."'" );
                        $send_to_name = $parent_user_detail->full_name;
                        $send_to_email= $parent_user_detail->email;
                        
                        ////Send Email to email Address
                        $to = $send_to_email;
                        $subject = 'New BDS message from '.$send_from_name;
                        $headers = array('Content-Type: text/html; charset=UTF-8');
                        $body = '<table><tr><td> Hi '.$send_to_name.' ,<br/>'.str_replace('<br>', '', $email).'</td></tr></table>' ;
...
//	nonroster_id
	if(!empty($nonroster_id)){
		foreach( explode(',', $nonroster_id) as $value){
                        $teacher_user_detail = $wpdb->get_row("SELECT * FROM $tbl_teacher WHERE id = '".$value."'" );
                        $send_to_name = $teacher_user_detail->full_name;
                        $send_to_email= $teacher_user_detail->email;
                        $_SESSION['to_email'] = $send_to_email;
                        ////Send Email to email Address
                        $to = $send_to_email;
                        $subject = 'New BDS message from '.$send_from_name;
                        $headers = array('Content-Type: text/html; charset=UTF-8');
	...
	echo json_encode(array('data'=> '1',
                        "email_to"=>$_SESSION['to_email']));
die();
}

function ajax_teacher_send_reply_message_request(){
...
//	roster_id
	if(!empty($to_email)){  
                ...
                ////Send Email to email Address
                $to = $to_email;
                $subject = 'New BDS Message from '.$from_name;
                $headers = array('Content-Type: text/html; charset=UTF-8');
                $body = '<table><tr><td> Hi '.$to_name.' ,<br/>'.str_replace('<br>', '', $message).'</td></tr></table>' ;
