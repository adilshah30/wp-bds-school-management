...
function inbox_message(){
	...
	$sql = "SELECT count(id) as new_msg FROM $table_rec WHERE `m_to` = '".$msg_to."' AND `status` = '0'";
	...
	<?php require_once TEACHER_PLUGIN_PATH . 'include/header.php'; ?>
                       ...
                       <div class="col-md-2 message-title">
                        ...
                        <div class="col-md-5 message-pagingation-wrap">
                            <div class="messages-pagination">
                                <?php                            
                                    $sql_paging = "SELECT $table_msg.*, $table_rec.m_from, $table_rec.m_to
						FROM $table_msg
							INNER JOIN $table_rec
							ON $table_msg.id=$table_rec.message_id
							AND $table_rec.m_to = '".$msg_to."'";
                                    $total_records = $wpdb->get_results( $sql_paging );
                                    $total_records= $wpdb->num_rows;
                                    $total_pages = ceil($total_records / $num_rec_per_page); 

                                    echo "<a href='".$site_url."/inbox-message/?p_no=1'>".'<< Prev'."</a> "; // Goto 1st page  
                                    for ($i=1; $i<=$total_pages; $i++) { 
                                                echo "<a href='".$site_url."/inbox-message/?p_no=".$i."'>".$i."</a> "; 
                                    } 
                                    echo "<a href='".$site_url."/inbox-message/?p_no=$total_pages'>".'Next >>'."</a> "; // Goto last page
                                ?>
                            </div>
                            
                        </div>
                        <div class="col-md-5 message-action-btn">
                            <p> <span><a href="<?php bloginfo('url')?>/sent-message"><i class="fa fa-arrow-right"></i> Sent </a></span>  <span class="em_active"><a href="<?php bloginfo('url')?>/inbox-message"><i class="fa fa-envelope"></i> Emails <b><?= $row->new_msg ?></b> </a></span> <span><a href="<?php bloginfo('url')?>/teacher-messages"><i class="fa fa-plus-circle"></i> New Email</a></span>  <input type="submit" class="btn-bds" name="delete_msg" value="Delete">
                               </p>
                        </div>
		...
		<div class="inbox_message">
			...
			<thead>
				...
				<th class="resp-display-none">Attachments</th>
                                <th>&nbsp;</th>
			</thead>

			<tbody>
			<?php
			   $sql = "SELECT $table_msg.*, $table_rec.message_id,$table_rec.m_from, $table_rec.m_to, $table_rec.id AS rec_id
						FROM $table_msg
							INNER JOIN $table_rec
							ON $table_msg.id=$table_rec.message_id
							AND $table_rec.m_to = '".$msg_to."' LIMIT $start_from , $num_rec_per_page";
                            $row = $wpdb->get_results( $sql );
			...
			?>
				<tr>
                                    ...
                                    <td class="resp-display-none"><?php echo date('m/d/y', strtotime($value->date)); ?></td>
                                    <td class="resp-display-none"><i class="fa fa-paperclip"></i> file</td>
                                    <td><a href="<?php echo site_url().'/reply-message/?msg_id='.$value->message_id.'&&from_id='.$value->m_from; ?>">Reply</a></td>
				</tr>
			<?php } ?>
